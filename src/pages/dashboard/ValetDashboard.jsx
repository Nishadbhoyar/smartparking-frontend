...
//     try {
//       // Note: Make sure your backend queue endpoint fetches BOTH 'VALET_REQUESTED' and 'RETURN_REQUESTED'
...
//     const config = getAuthConfig();
//     const valetId = localStorage.getItem("userId");
//     if (!config || !valetId) return;

//     try {
//       if (booking.status === "VALET_REQUESTED") {
//         // Assign this valet to the booking → status becomes VALET_ASSIGNED
//         const res = await axios.post(
//           `http://localhost:8080/api/bookings/${booking.id}/assign?valetId=${valetId}`,
//           {},
//           config
//         );
//         // The endpoint returns BookingDTO directly
//         setActiveTask(res.data);
//       } else if (booking.status === "RETURN_REQUESTED") {
//         // ✅ FIX: Fetch fresh booking from server so we have the latest status + dropoffOtp
...
//     } catch (err) {
//       console.error(err);
//       alert("Error accepting task: " + (err.response?.data || err.message));
//     }
//   };

//   // Verifies the OTP with the backend for Pickup or Dropoff
//   const handleVerifyOtp = async (type) => {
//     if (otpInput.length < 4) return alert("Please enter the full OTP.");
//     setSubmitting(true);
//     const config = getAuthConfig();

//     try {
//       // ✅ Fixed: endpoints are /verify-pickup and /verify-dropoff
//       const endpoint = type === 'pickup' ? 'verify-pickup' : 'verify-dropoff';
//       const res = await axios.post(
//         `http://localhost:8080/api/bookings/${activeTask.id}/${endpoint}`,
//         { otp: otpInput },
//         config
//       );
      
//       if (type === 'dropoff') {
//          alert("✅ Keys handed over successfully. Task complete!");
//          setActiveTask(null);
//       } else {
//          alert("✅ Pickup Verified! Please proceed to park the vehicle.");
//          setActiveTask(res.data.booking || res.data);
//       }
//       setOtpInput("");
//       fetchQueue();
//     } catch (err) {
//       alert(err.response?.data || err.response?.data?.error || "Invalid OTP entered.");
//     } finally {
//       setSubmitting(false);
//     }
//   };
...
//   // Finalizes parking (Uploads coordinates + Images)
...
//       // ✅ BUG FIX: backend expects param name "file" (singular), not "files"
//       // Also: do NOT manually set Content-Type — let the browser set it with
//       // the correct multipart boundary automatically.
//       if (completionData.images.length > 0) {
//         formData.append("file", completionData.images[0]);
//       }
...
//                     {/* STATE 1: WAITING FOR PICKUP OTP */}
//                     {activeTask.status === "VALET_ASSIGNED" && (
//                         <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:24, textAlign:"center" }}>
//                           <div style={{ width:56, height:56, borderRadius:16, background:C.amberDim, margin:"0 auto 16px", display:"flex", alignItems:"center", justifyContent:"center" }}>
//                              <Key size={24} color={C.amber} />
//                           </div>
//                           <h4 className="df" style={{ fontSize:18, fontWeight:700, color:C.white, marginBottom:8 }}>Verify Pickup</h4>
//                           <p style={{ fontSize:13, color:C.muted, marginBottom:20 }}>Ask the customer for their pickup OTP to receive the keys.</p>
//                           <input
//                             type="text"
//                             maxLength={6}
//                             value={otpInput}
...
//                             placeholder="------"
//                             style={{
//                               width:"160px", fontSize:28, letterSpacing:"6px", textAlign:"center",
//                               background:C.bg, border:`1px solid ${C.border}`, borderRadius:12, padding:"12px",
//                               color:C.white, fontWeight:800, marginBottom:20, outline:"none"
//                             }}
//                           />
//                           <button className="btn-primary" disabled={submitting} onClick={() => handleVerifyOtp('pickup')} style={{ width:"100%", justifyContent:"center" }}>
//                             {submitting ? "Verifying..." : "Verify & Pick Up Vehicle"}
//                           </button>
//                         </div>
//                     )}

//                     {/* STATE 1b: RETURN_REQUESTED — valet must verify dropoff OTP to hand keys back */}
//                     {activeTask.status === "RETURN_REQUESTED" && (
//                         <div style={{ background:C.surface, border:`1px solid rgba(75,158,255,0.25)`, borderRadius:16, padding:24, textAlign:"center" }}>
//                           <div style={{ width:56, height:56, borderRadius:16, background:C.blueDim, margin:"0 auto 16px", display:"flex", alignItems:"center", justifyContent:"center" }}>
//                              <Key size={24} color={C.blue} />
//                           </div>
//                           <h4 className="df" style={{ fontSize:18, fontWeight:700, color:C.white, marginBottom:8 }}>Verify Drop-off</h4>
//                           <p style={{ fontSize:13, color:C.muted, marginBottom:20 }}>Ask the customer for their drop-off OTP before handing back the keys.</p>
//                           <input
//                             type="text"
//                             maxLength={6}
//                             value={otpInput}
...
//                             }}
//                           />
//                           <button className="btn-blue" disabled={submitting} onClick={() => handleVerifyOtp('dropoff')} style={{ width:"100%", justifyContent:"center" }}>
//                             {submitting ? "Verifying..." : "Confirm & Hand Back Keys"}
//                           </button>
//                         </div>
//                     )}

//                     {/* STATE 2: PICKED UP -> NEED TO PARK & UPLOAD PROOF */}
//                     {activeTask.status === "VALET_PICKED_UP" && (
//                       !showCompleteModal ? (
//                         <div>
...
//                             {[
//                               { icon:<MapPin size={16}/>, color:C.blue, text:"Drive vehicle to designated parking lot" },
//                               { icon:<Shield size={16}/>, color:"#C084FC", text:"Park safely and upload photo proof" },
//                             ].map((step, i) => (
...
//                             <CheckCircle2 size={18}/> Process Parking
//                           </button>
//                         </div>
//                       ) : (
//                         /* Parking Completion Form */
...
//                             {submitting ? <><Loader2 size={16} style={{ animation:"spin 1s linear infinite" }}/> Saving...</> : "Submit & Finish Parking"}
//                           </button>
//                         </div>
//                       )
//                     )}

//                     {/* STATE 3: WAITING FOR DROPOFF OTP */}
//                     {activeTask.status === "RETURN_REQUESTED" && (
//                         <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:16, padding:24, textAlign:"center" }}>
//                           <div style={{ width:56, height:56, borderRadius:16, background:C.purpleDim, margin:"0 auto 16px", display:"flex", alignItems:"center", justifyContent:"center" }}>
//                              <Key size={24} color={C.purple} />
//                           </div>
//                           <h4 className="df" style={{ fontSize:18, fontWeight:700, color:C.white, marginBottom:8 }}>Verify Dropoff</h4>
//                           <p style={{ fontSize:13, color:C.muted, marginBottom:20 }}>Ask the customer for their Dropoff OTP before handing over the keys.</p>
//                           <input
//                             type="text"
//                             maxLength={6}
//                             value={otpInput}
...
//                             }}
//                           />
//                           <button className="btn-primary" disabled={submitting} onClick={() => handleVerifyOtp('dropoff')} style={{ width:"100%", justifyContent:"center", background: C.purple, color: "#fff" }}>
//                             {submitting ? "Verifying..." : "Verify & Handover Keys"}
//                           </button>
//                         </div>
//                     )}
//                   </div>
...
//                     {queue.map(booking => {
//                       const isReturn = booking.status === "RETURN_REQUESTED";
                      
//                       return (
//                       <div key={booking.id} className="queue-item">
//                         <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
//                           <div style={{ display:"flex", alignItems:"center", gap:10 }}>
//                             <div className="icon-box" style={{ background: isReturn ? C.purpleDim : C.blueDim }}>
//                                <Car size={16} color={isReturn ? C.purple : C.blue}/>
//                             </div>
//                             <div>
...
//                           </div>
//                           <span style={{ 
//                             padding:"4px 10px", borderRadius:100, fontSize:10, fontWeight:700, fontFamily:"Syne,sans-serif",
//                             background: isReturn ? C.purpleDim : C.amberDim, 
//                             color: isReturn ? C.purple : C.amber 
//                           }}>
//                             {isReturn ? "Dropoff" : "Pickup"}
//                           </span>
//                         </div>
//                         <div style={{ marginBottom:12, display:"flex", flexDirection:"column", gap:4 }}>
//                           <p style={{ fontSize:13, color:C.muted }}><strong style={{ color:C.text }}>Vehicle:</strong> {booking.vehicleNumber}</p>
//                           <p style={{ fontSize:13, color:C.muted }}><strong style={{ color:C.text }}>Location:</strong> {isReturn ? "Parking Lot" : "Customer Location"}</p>
//                         </div>
//                         <button onClick={() => handleAccept(booking)} disabled={!!activeTask}
//                           className="btn-accept"
//                           style={{
//                             background: activeTask ? "rgba(107,117,148,0.1)" : (isReturn ? C.purpleDim : C.accentDim),
//                             color: activeTask ? C.muted : (isReturn ? C.purple : C.accent),
//                             border: `1px solid ${activeTask ? C.border : (isReturn ? "rgba(192,132,252,0.2)" : "rgba(184,255,87,0.2)")}`,
//                             cursor: activeTask ? "not-allowed" : "pointer",
//                           }}>
//                           {activeTask ? "Complete Current Task First" : (isReturn ? "Process Dropoff" : "Accept Pickup")}
//                         </button>
//                       </div>
//                     )})}
//                   </div>
...
//           {/* ─── HEADER ─── */}
//           <div style={{ display:"flex", flexWrap:"wrap", alignItems:"flex-start", justifyContent:"space-between", gap:20, marginBottom:28 }} className="vd-header">
//             <div>
//               <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:10 }}>
//                 <div style={{ padding:"6px 12px", background:C.accentDim, border:`1px solid rgba(184,255,87,0.2)`, borderRadius:8, display:"flex", alignItems:"center", gap:6 }}>
//                   <Navigation size={13} color={C.accent}/>
...
//           {/* ─── STAT CARDS ─── */}
//           <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))", gap:14, marginBottom:28 }} className="vd-stat-grid">
//             {statItems.map(s => (
//               <div key={s.label} className="stat-card">
//                 <div style={{ display:"flex", alignItems:"center", gap:14 }}>
//                   <div style={{ width:44, height:44, borderRadius:13, background:s.dim, border:`1px solid ${s.color}28`, display:"flex", alignItems:"center", justifyContent:"center", color:s.color }}>
...
//           {/* ─── MAIN GRID ─── */}
//           <div style={{ display:"grid", gridTemplateColumns:"2fr 1fr", gap:20 }} className="vd-main-grid">
...
//                   <div style={{ padding:24 }}>
//                     {/* Customer + Vehicle Info */}
//                     <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginBottom:24 }} className="vd-info-grid">
//                       {[
//                         { label:"Customer", value:activeTask.user?.name || "Guest", sub:activeTask.contactNumber || "N/A", icon:<User size={16}/>, color:C.blue, dim:C.blueDim },
//                         { label:"Vehicle", value:activeTask.vehicleNumber, sub:activeTask.vehicleModel || "Model: N/A", icon:<Car size={16}/>, color:C.accent, dim:C.accentDim },
//                       ].map(card => (
...
//                         <div style={{ marginBottom:20 }}>
//                           <span className="loc-label">2. Upload Proof</span>
//                           <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8 }} className="vd-img-grid">
//                             {imagePreviews.map((src, i) => (
//                               <div key={i} style={{ position:"relative", aspectRatio:"1", borderRadius:10, overflow:"hidden", border:`1px solid ${C.border}` }}>
//                                 <img src={src} alt="proof" style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
//                                 <button onClick={() => removeImage(i)}
...
import ValetNavbar from "../../components/navbar/ValetNavbar";
