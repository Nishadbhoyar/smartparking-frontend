...
//     if (token) {
//       setVerifying(true); // Switch UI to Verification Mode
      
//       axios.post("https://smartparking-backend-1.onrender.com/api/auth/verify-magic-link", { token })
//         .then((response) => {
//           const { token: jwt, redirectUrl, user } = response.data;
...
//           }, 1500); // Slight delay so user sees the "Success" state
//         })
//         .catch((err) => {
//           console.error("Magic Link Error:", err);
//           alert("❌ Invalid or Expired Magic Link. Please try again.");
//           setVerifying(false); // Go back to login form
...
function Login() {
  ...
  const [step, setStep] = useState(1);
  const [loading, setLoading] = useState(false);
  const [verifying, setVerifying] = useState(false); // NEW: State for Magic Link Verification

  /* ─── NEW: Magic Link Verification Logic ─── */
  /* ─── MAGIC LINK VERIFICATION LOGIC ─── */
  useEffect(() => {
    const token = searchParams.get("token");

    if (token) {
      setVerifying(true); // Show loading UI
      ...
      axios.post("https://smartparking-backend-1.onrender.com/api/auth/verify-magic-link", { token })
        ...
        .catch((err) => {
          ...
          setVerifying(false); 
          navigate("/login"); // Clear token from URL on failure
        });
    }
  ...
  const validateEmail = (email) => {
    const lowerEmail = email.toLowerCase();
    if (lowerEmail.includes("@gamil.com") || lowerEmail.includes("@gmai.com")) return "Did you mean @gmail.com?";
    if (lowerEmail.includes("@yaho.com")) return "Did you mean @yahoo.com?";
    return null;
  };
            ...
            {verifying ? (
              ...
              >
                <div style={{ position: "relative", display: "inline-block", marginBottom: 20 }}>
                    <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: C.accent, opacity: 0.2, filter: "blur(20px)" }} />
                    <Loader2 size={48} color={C.accent} className="animate-spin" style={{ position: "relative" }} />
                </div>
                <h2 className="display-font" style={{ color: C.white, fontSize: 24, fontWeight: 700, marginBottom: 12 }}>
                  Verifying Link...
                </h2>
                <p style={{ color: C.muted, fontSize: 14, lineHeight: 1.5 }}>
                  Please wait while we securely log you into your dashboard.
                </p>
              </motion.div>
