...
import { Navigation, Home, LogOut, Menu, X, User, Lock, Mail, Eye, EyeOff, Check, AlertCircle } from "lucide-react";
...
function ValetNavbar() {
  ...
  /* Valet uses a slightly warmer lime to match ValetDashboard's #B8FF57 */
  const ACCENT = "#B8FF57";
  ...
  return (
    <>
      <nav style={{
        ...S.nav,
        background:     scrolled ? "rgba(2,3,8,0.97)"  : "transparent",
        boxShadow:      scrolled ? "0 1px 0 #1a1e2e"   : "none",
        backdropFilter: scrolled ? "blur(16px)"          : "none",
        padding:        scrolled ? "12px 0"              : "20px 0",
      }}>
        <div style={S.inner}>
          {/* LOGO */}
          <div onClick={() => navigate("/valet-dashboard")} style={S.logo}>
            <div style={S.logoIconWrap}>
              <div style={{ ...S.logoGlow, background: `radial-gradient(circle, ${ACCENT}60 0%, transparent 70%)` }} />
              <div style={{ ...S.logoIcon, background: ACCENT }}><Navigation size={20} color="#000" /></div>
            </div>
            <span style={S.logoText}>Valet<span style={{ color: ACCENT }}>Service</span></span>
            <span style={{ ...S.badge, background: `${ACCENT}15`, borderColor: `${ACCENT}30`, color: ACCENT }}>VALET</span>
          </div>

          {!isMobile && (
            <div style={S.desktopNav}>
              <NavItem to="/valet-dashboard" accent={ACCENT}><Home size={15} /> Dashboard</NavItem>
              <button
                onClick={() => setShowProfile(true)}
                style={{ ...S.profileBtn, background: `${ACCENT}12`, borderColor: `${ACCENT}25`, color: ACCENT }}
        ...
        {isMobile && isOpen && (
          <div style={S.mobileMenu}>
            <MobileNavItem to="/valet-dashboard" icon={<Home size={18} />} accent={ACCENT}>Dashboard</MobileNavItem>
            <button onClick={() => { setShowProfile(true); setIsOpen(false); }}
              style={{ ...S.mobileProfileBtn, background: `${ACCENT}12`, borderColor: `${ACCENT}25`, color: ACCENT }}>
      ...
      {showProfile && <ProfileModal onClose={() => setShowProfile(false)} accentColor={ACCENT} />}
    </>
  );
}
...
function ProfileModal({ onClose, accentColor }) {
  ...
  const initial = (stored.name || stored.email || "V")[0].toUpperCase();
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ position: "relative" }}>
        <input value={value} onChange={e => onChange(e.target.value)} type={show ? "text" : "password"} placeholder="••••••••"
          style={{ ...M.input, paddingRight: 44, borderColor: match === true ? "#2d7a2d" : match === false ? "#7a2d2d" : "#2e2e2e" }}
          onFocus={e => (e.target.style.borderColor = accent)}
...
function MobileNavItem({ to, icon, children, accent }) {
  return (
    <NavLink to={to} style={({ isActive }) => ({
...
const S = {
  ...
  logoGlow: { position: "absolute", inset: 0, borderRadius: 10, filter: "blur(6px)" },
  logoIcon: { position: "relative", width: 38, height: 38, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center" },
  logoText: { fontSize: 18, fontWeight: 800, color: "#fff", letterSpacing: "-0.5px" },
  badge: { fontSize: 10, fontWeight: 800, letterSpacing: "0.1em", padding: "3px 8px", borderRadius: 99, border: "1px solid" },
  desktopNav: { display: "flex", alignItems: "center", gap: 8 },
  profileBtn: { display: "flex", alignItems: "center", gap: 7, border: "1px solid", borderRadius: 10, padding: "9px 16px", fontWeight: 600, fontSize: 14, cursor: "pointer", transition: "background 0.15s" },
  ...
  mobileMenu: { maxWidth: 1400, margin: "0 auto", padding: "12px 24px 16px", borderTop: "1px solid #1a1e2e", background: "rgba(2,3,8,0.98)", backdropFilter: "blur(16px)" },
  mobileProfileBtn: { display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "12px 16px", border: "1px solid", borderRadius: 12, fontWeight: 700, fontSize: 14, cursor: "pointer", marginBottom: 6 },
...
const M = {
  ...
  modal:   { background: "#0d0d0d", border: "1px solid #1e2230", borderRadius: 20, width: "100%", maxWidth: 440, overflow: "hidden", boxShadow: "0 24px 80px rgba(0,0,0,0.7)" },
